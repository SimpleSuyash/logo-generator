...
function validateText(textInput){
    ...
    if(textInput){
        ...
        //when input is not empty strings
            ...
            // when logo text is too long 
            if(text.length > 3){
                throw new Error("Invalid logo text: Value entered is too long.");
            //when logo text is between 1 to 3 characters
            }else{
                logoText= text;
            }
        }
...
//validates the input for colour value
function validateColour(colourInput){
    ...
    if(colourInput){
        ...
        //if entered value is 
        //either a valid css defined colour name or a valid hex value
